...
class Covid_NLI_B_Processor(DataProcessor):
    """Processor for the COVID data set."""
...
class Covid_NLI_M_Processor(DataProcessor):
    """Processor for the COVID data set."""
    ...
    def get_labels(self):
        """See base class."""
        return ['unrelated', 'negative', 'neutral', 'positive']
        ...
        for (i, line) in enumerate(lines):
          #  if i>50:break
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
class Covid_NLI_M_Sampled_Processor(DataProcessor):
    """Processor for the COVID data set."""

    def get_train_examples(self, data_dir):
        """See base class."""
        train_data = pd.read_csv(os.path.join(data_dir, "train_NLI_M_Sampled.csv"),header=None,sep="\t").values
        return self._create_examples(train_data, "train")
        ...
        for (i, line) in enumerate(lines):
          #  if i>50:break
            ...
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=text_b, label=label))
        return examples
class Covid_QA_B_Processor(DataProcessor):
    """Processor for the COVID data set."""
